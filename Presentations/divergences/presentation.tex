...
\begin{frame}
  ...
  {} & Reparameterized \\
  \midrule
  $\mathcal{N}(\mu, \sigma)$   &  $\mu + \sigma \odot \mathcal{N}(0, 1)$ \\
  \\
  $\mathcal{U}(a, b)$   &  $a + (b - a) \odot \mathcal{U}(0, 1)$ \\
  \\
  $\text{Exp}(\lambda)$   &  $\text{Exp}(1) / \lambda$ \\
  \\
  $\text{Cauchy}(\mu, \gamma)$   &  $\mu + \gamma \odot \text{Cauchy}(0, 1)$ \\
  \bottomrule
  \end{tabular}
  \end{center}
\end{frame}
...
\begin{frame}
  ...
  $\text{Cauchy}(\mu, \gamma)$   &  $\mu + \gamma \odot \text{Cauchy}(0, 1)$ \\
  \\
  \multirow{2}{*}{$\text{Laplace}(\mu, b)$}  &  $u \sim \mathcal{U}(-1, 1)$ \\
                                          & $\mu - b \odot \text{sgn}(u) \odot \ln \big[-|u|+1 \big]$ \\
  \bottomrule
  \end{tabular}
  \end{center}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item Forward Kullback-Leibler
      \begin{itemize} \item Maximum Likelihood Estimation \end{itemize}
    \item Reverse Kullback-Leibler
      \begin{itemize} \item Variational Autoencoders \item Variational Inference \end{itemize}
    \item Jensen-Shannon Divergence
      \begin{itemize} \item Generative Adversarial Network (Original) \end{itemize}
    \item Earth Mover's Distance
      \begin{itemize} \item Wasserstein GAN (WGAN) \end{itemize}
    \item Pearson $\chi^2$ Divergence
      \begin{itemize} \item Least Squares GAN (LSGAN) \end{itemize}
  \end{itemize}
\end{frame}
