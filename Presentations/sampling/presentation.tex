...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Uses velocity $v$ and position $x$:
  \end{itemize}
  \begin{gather*}
    H(x, v) = U(x) + K(v) \\
    U(x) = - \log f(x) \\
    K(v) = \sum_i \frac{v_i^2}{2} \\
    \frac{d x_i}{d t} = \frac{\partial H}{\partial v_i} = v_i \\
    \frac{d v_i}{d t} = -\frac{\partial H}{\partial x_i} = -\frac{\partial U(x_i)}{\partial x_i} \\
    r = \exp \left( -H(x, v) + H(x_s, v_s) \right)
  \end{gather*}
\end{frame}
...
\begin{frame}
  ...
  \begin{algorithmic}[1]
    \REQUIRE Model $F$, Stepsize $\epsilon$, Leapfrog Steps $L$, Current State $x_s$
      \STATE Set $x = x_s$, $v \sim \mathcal{N}(0, 1)$, $v_s = v$
      \STATE $v = v - \epsilon \frac{\partial U (x)}{\partial x} / 2$
      \FOR {$l = 0, 1, \hdots L$}
        \STATE $x = x + \epsilon \cdot v$
        \STATE $v = v - \epsilon \frac{\partial U (x)}{\partial x}$ except at end of trajectory
      \ENDFOR
      \STATE $v = v -\epsilon \frac{\partial U (x)}{\partial x} / 2$
      \STATE $v = -v$ to make proposal symmetric
      \STATE Compute acceptance probability
        \begin{nospaceflalign*}
          r = \exp \left( U(x_s) - U(x) + K(v_s) - K(v)  \right) &&
        \end{nospaceflalign*}
      \STATE Sample $u \sim \text{Uniform}(0, 1)$
      \IF {$u < r$}
        \RETURN {$x$}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \end{itemize}
    \item Has trouble sampling from distributions with isolated local minimums (lack of energy to corss the energy barrier)
